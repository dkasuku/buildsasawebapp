...
const NAV_GROUPS: NavGroup[] = [
  ...
  },
  {
    label: "Resources",
    items: [
      { key: "equipment", label: "Equipment Inventory", icon: Truck },
    ],
  },
  {
    label: "Reports",
    ...
    <>
      ...
      >
        <div className="px-5 py-5 border-b border-[#222A35] flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[#FF6B1A] to-[#FF8A4A] flex items-center justify-center">
            <HardHat className="w-[18px] h-[18px] text-white" strokeWidth={2.4} />
          </div>
          <div className="flex-1">
            <div className="text-[14px] text-white tracking-tight font-display">ConstructAI</div>
          ...
          {menuOpen && (
            <>
              ...
              <div className="absolute z-20 left-3 right-3 bottom-[68px] rounded-lg border border-[#222A35] bg-[#11161D] shadow-2xl py-1">
                ...
                <button onClick={() => { setMenuOpen(false); onClose(); window.dispatchEvent(new Event("buildflex:start-tour")); }} className="w-full flex items-center gap-2.5 px-3 py-2 text-[13px] text-[#8A95A5] hover:text-white hover:bg-[#161C24]"><Sparkles className="w-4 h-4" /> Take a tour</button>
