...
export const api = {
  ...
  verifyEmail: (token: string) => http<{ ok: boolean }>("/api/auth/verify-email", { method: "POST", body: JSON.stringify({ token }) }),
  resendVerification: () => http<{ ok: boolean }>("/api/auth/resend-verification", { method: "POST" }),
  ...
  payBillingInvoice: (id: string) => http<{ authorizationUrl?: string; reference?: string; demo?: boolean; comp?: boolean; message?: string; ok?: boolean; alreadyPaid?: boolean }>(`/api/billing/invoices/${id}/pay`, { method: "POST" }),
